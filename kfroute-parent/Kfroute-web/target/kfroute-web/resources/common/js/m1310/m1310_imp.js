...
function dataLoad(){//加载数据到数据库，此时数据仅存放在临时表中，用户提交后才会更新到审批表等待审批
	...
	if(imp_form.data_load.value=="重新导入"){
		layer.confirm('导入文件将删除，确定？', {icon: 3,title:'提示'}, function(index){
			...
			$.post(ctx+"/m1310/m1310_delUploadFile.do",function(data){
				if(data==1){
	...
	layer.confirm('确定导入吗？', {icon: 3,title:'提示'}, function(index){
		...
		$.ajaxFileUpload({
			url:ctx+"/m1310/m1310_uploadAndCheck.do",//服务器端程序
			secureuri:false,
...
function mubanDownload(){//下载模板到本地
	 this.location = ctx+"/m1310/m1310_downloadModel.do?filename=device_imp_model.xlsx";
...
function showImp_detail(){
	...
	this.location = ctx+"/m1310/m1310_downloadUploadDetail.do";
}
function submit(){
	if(!commit_flag)
		return;
