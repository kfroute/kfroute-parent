...
public class AlipayCore {
     ...
     * @return 签名结果字符串
     */
    public static String buildAlipayMysign(Map<String, String> sArray,String creditkey) {
        String prestr = createLinkString(sArray); //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
        prestr = prestr + creditkey; //把拼接后的字符串再与安全校验码直接连接起来
        String mysign = AlipayMd5Encrypt.md5(prestr);
        return mysign;
    }
     ...
     * @return 签名结果字符串
     */
    public static String buildAlipayMysignByGBK(Map<String, String> sArray,String creditkey) {
        String prestr = createLinkString(sArray); //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
        prestr = prestr + creditkey; //把拼接后的字符串再与安全校验码直接连接起来
        String mysign = AlipayMd5Encrypt.md5ByGBK(prestr);
        return mysign;
    }
