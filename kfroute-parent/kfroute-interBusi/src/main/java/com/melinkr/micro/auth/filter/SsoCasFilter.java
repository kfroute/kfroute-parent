...
public class SsoCasFilter extends AuthenticatingFilter {
	...
	protected boolean onLoginSuccess(AuthenticationToken token,
		...
		ServletContext context = ((HttpServletRequest) request).getSession()
				.getServletContext();
		if (context != null) {// shiroSession可能为空
			TicketSubjectMap ticketSubjectMap = (TicketSubjectMap) context
					.getAttribute("ticketSubjectMap");
			if (ticketSubjectMap == null) {
				ticketSubjectMap = new TicketSubjectMap();
			}
			ticketSubjectMap.addSubject(ticket, subject);
			context.setAttribute("ticketSubjectMap", ticketSubjectMap);
		}
