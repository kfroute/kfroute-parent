...
import com.melinkr.micro.alipay.util.AlipayCore;
...
public class AlipayNotify {
     ...
     * @return 验证结果
     */
    public static boolean verifyAlipay(Map<String, String> params,Map<String ,String>  secretMap) {

    	String secretKey=secretMap.get("key");
        String mysign = getAlipayMysign(params,secretKey);
        String responseTxt = "true";
        if(params.get("notify_id") != null) {responseTxt = verifyAlipayResponse(params.get("notify_id"),secretMap.get("partner"));}
        String sign = "";
        if(params.get("sign") != null) {sign = params.get("sign");}
     ...
     * @return 生成的签名结果
     */
    private static String getAlipayMysign(Map<String, String> Params,String secretKey) {
        Map<String, String> sParaNew = AlipayCore.paraFilter(Params);//过滤空值、sign与sign_type参数
        String mysign = AlipayCore.buildAlipayMysign(sParaNew,secretKey);//获得签名结果
        return mysign;
    }
